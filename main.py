...
# Fixed events for each month (You update this manually)
fixed_events = {
    "January": ["New Year Celebration", "Winter Festival"],
    "February": ["Science Fair", "Valentine's Day Event"],
    "March": ["Spring Concert", "Math Olympiad"],
    "April": ["Art Exhibition", "Earth Day Activities"],
    "May": ["Sports Day", "Graduation Ceremony"],
    "June": ["Summer Camp", "Music Workshop"],
    "July": ["Coding Bootcamp", "Summer Festival"],
    "August": ["Back to School Orientation", "Book Fair"],
    "September": ["Autumn Festival", "Science Workshop"],
    "October": ["Halloween Party", "STEM Challenge"],
    "November": ["Thanksgiving Celebration", "Robotics Competition"],
    "December": ["Christmas Party", "Year-End Gala"]
}
...
def show_events(message):
    """Displays events based on the selected month."""
    chat_id = message.chat.id
    month = message.text

    events = fixed_events.get(month, [])
    if events:
        events_text = f"🎉 **Events for {month}:**\n" + "\n".join(f"- {event}" for event in events)
    else:
        events_text = f"❌ No events available for {month}."

    bot.send_message(chat_id, events_text)
